package com.mark.demo.shiro.mybatis.dialect;
...
public class SQLServer2005Dialect implements Dialect
{
    ...
    private String getLimitString(String querySqlString, int offset, int limit, String limitPlaceholder)
    {
        ...
        if (org.apache.commons.lang3.StringUtils.isEmpty(orderby)) orderby = "ORDER BY CURRENT_TIMESTAMP";
        ...
        result.append("WITH query AS (SELECT ").append(distinctStr).append("TOP 100 PERCENT ").append(" ROW_NUMBER() OVER (").append(orderby)
                .append(") as __row_number__, ").append(pagingBuilder).append(") SELECT * FROM query WHERE __row_number__ BETWEEN ").append(offset).append(" AND ")
    ...
    static String getOrderByPart(String sql)
    {
        ...
        if (orderByIndex != -1) // if we find a new "order by" then we need to ignore
